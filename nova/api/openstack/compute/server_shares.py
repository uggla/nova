    ...
    def create(self, req, server_id, body):
        ...
        with nova_context.target_cell(context, im.cell_mapping) as cctxt:
            ...
            try:
                ...
                manila_share_data = self.manila.get(share_id)
                ...
                # Reload the share status, if the compute as finished
                # mounting the share on the compute the status will be
                # "inactive" or "error" If mounting is not completed
                # share status will be "attaching".
                share_mapping = (
                    objects.ShareMapping.get_by_instance_uuid_and_share_id(
                        cctxt, server_id, manila_share_data.id
                    )
                )
