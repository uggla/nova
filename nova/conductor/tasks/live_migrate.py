    ...
    def _check_can_migrate_pci(self, src_host, dest_host):
        ...
        for pci_request in self.instance.pci_requests.requests:
            if pci_request.source == objects.InstancePCIRequest.FLAVOR_ALIAS:
                devs = self.instance.get_pci_devices(request_id=pci_request.request_id)
                if not all([dev.extra_info['live_migratable'] for dev in devs]):
                    # allow only migratable devices for flavor-based ones.
                    raise exception.MigrationPreCheckError(
                        reason= "non live-migratable related PCI devices for instance "
