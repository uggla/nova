...
class ComputeManager(manager.Manager):
    ...
    def check_can_live_migrate_destination(self, ctxt, instance,
        ...
        try:
            ...
            # TODO(Uggla): Reminder:
            # Assuming scheduler has a filter in place.
            # Based on scheduler request we know that we have a dest instance with the correct number of live_migratable pci devices + other pci devices (nics).
            # So now we just need to claim the pci migratable devices.
            pci_reqs = []
            for pci_request in instance.pci_requests.requests:
                if pci_request.source == objects.InstancePCIRequest.FLAVOR_ALIAS:
                    # TODO(Uggla) : Check that the alias has live_migratable flag
                    pci_reqs.append(pci_request)
            if pci_reqs:
                # Create PCI requests and claim against PCI resource tracker
                lm_pci_requests = objects.InstancePCIRequests(
                    requests=pci_reqs,
                    instance_uuid=instance.uuid)
                pci_devs = objects.PciDeviceList.get_by_instance_uuid(ctxt, instance.uuid)
                src_pci_devs = [pci_dev for pci_dev in pci_devs if pci_dev.extra_info['live_migratable']]
                claimed_lm_pci_devs = self._claim_from_pci_reqs(ctxt, instance, lm_pci_requests)

                if len(src_pci_devs) != len(claimed_lm_pci_devs):
                    raise exception.LiveMigrationSrcDstMismatch(
                        dev_src=len(src_pci_devs), dev_dst=len(claimed_lm_pci_devs))

                # TODO(Uggla) Passing thru the whole PCI device infos is maybe too large
                migrate_data.pci_dev_map_src_dst = dict(zip(list(map(lambda dev: dev.address, src_pci_devs)), list(map(lambda dev: dev.address, claimed_lm_pci_devs))))
