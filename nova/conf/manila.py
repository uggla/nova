...
Note: Nova does not support the Manila v2 API since the Nova 17.0.0 Queens
release.
...
zone in Manila as the instance availability zone in Nova.
...
If that AZ is not in Manila (or ``allow_availability_zone_fallback=False`` in
manila.conf), the volume create request will fail and the instance will fail
the build request.
