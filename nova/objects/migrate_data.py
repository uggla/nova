...
class LibvirtLiveMigrateData(LiveMigrateData):
    ...
    fields = {
        ...
        'dst_cpu_shared_set_info': fields.SetOfIntegersField(nullable=True),
    }
    ...
    def obj_make_compatible(self, primitive, target_version):
        super(LibvirtLiveMigrateData, self).obj_make_compatible(
            primitive, target_version)
        target_version = versionutils.convert_version_to_tuple(target_version)
        if (target_version < (1, 12) and
                'dst_cpu_shared_set_info' in primitive):
            del primitive['dst_cpu_shared_set_info']
