...
class ShareMapping(base.NovaPersistentObject, base.NovaObject):
    ...
    def attach(self):
        ...
        self._change_status(fields.ShareMappingStatus.ACTIVE)
    ...
    def detach(self, force=False):
        if force:
            ...
            "Force removal share '%s' from instance '%s'.",
                self.share_id, self.instance_uuid)
            self._detach()
        elif self.status == fields.ShareMappingStatus.ACTIVE:
            LOG.info(
            "Share '%s' about to be detached from instance '%s'.",
                self.share_id, self.instance_uuid)
            self._change_status(fields.ShareMappingStatus.INACTIVE)
        else:
            LOG.info(
...
class ShareMappingList(base.ObjectListBase, base.NovaObject):
    ...
    def convert_to_driver(self, context, instance):
        share_drv_list = ShareMappingDrvList()
        for share in self:
            share_mapping_driver = (
                ShareMappingLibvirt.from_share_mapping(
                    context, instance, share)
            )
            share_drv_list.append(share_mapping_driver)

        return share_drv_list


class ShareMappingDrvList(list):
    def mount_all(self):
        for share in self:
            share.attach()
...
class ShareMappingLibvirtNFSBuilder(ShareMappingLibvirtFactory):
    def build(self, context, instance, share_mapping):
        return ShareMappingLibvirtNFS(context).convert(
            instance,
            share_mapping
        )
...
class ShareMappingLibvirt(ShareMapping, metaclass=ABCMeta):
    ...
    def from_share_mapping(cls, context, instance, share_mapping):
        """Downcast a ShareMapping object to the proper driver
        ShareMappingLibvirt<driver> child object based on the share_mapping
        prototype.
        """
    ...
    def convert(self, instance, share_mapping):
...
class ShareMappingLibvirtNFS(ShareMappingLibvirt):
    ...
    def attach(self):
        try:
            ...
            self._change_status(fields.ShareMappingStatus.ERROR)
            raise exception.ShareMountError(
                share_id=self.share_id,
                server_id=self.instance_uuid,
                reason=exc
            )
