...
class _TestShareMappingList(object):
    ...
    def test_mount_all(self):
        ...
        with mock.patch('nova.objects.share_mapping.'
                        'ShareMappingLibvirtNFS.mount') as mock_attach:
    ...
    def test_umount_all(self):
        share_mappings_drv = self.test_convert_to_driver_two_items()
        with mock.patch('nova.objects.share_mapping.'
                        'ShareMappingLibvirtNFS.umount') as mock_detach:
    ...
    def test_mount_umount_empty_list(self):
        ...
        with mock.patch(
            'nova.objects.share_mapping.'
            'ShareMappingDrvList.mount_all'
        ) as mock_mount, mock.patch(
            'nova.objects.share_mapping.'
            'ShareMappingDrvList.umount_all'
            ...
            mock_umount.assert_called()
            self.assertEqual(mock_umount.call_count, 1)

    def test_attach_detach_empty_list(self):
        share_mappings_drv = self.test_convert_to_driver_no_items()
        with mock.patch(
            'nova.objects.share_mapping.'
            'ShareMappingDrvList.attach_all'
        ) as mock_attach, mock.patch(
            'nova.objects.share_mapping.'
            'ShareMappingDrvList.detach_all'
        ) as mock_detach:
            ...
            self.assertEqual(mock_detach.call_count, 1)
...
class TestShareMappingLibvirtNFS(test_objects._LocalTest):
    ...
    def test_mount_and_attach(self, mock_drv):
        ...
        with mock.patch('nova.db.main.api.share_mapping_update') as mock_upd:
            ...
            share_mapping_libvirt_nfs.mount()
    ...
    @mock.patch(
        'nova.db.main.api.share_mapping_delete_by_instance_uuid_and_share_id')
    def test_umount_and_detach(self, mock_del, mock_drv):
        share_mapping_libvirt_nfs = self.test_mount_and_attach()
        ...
        with mock.patch('nova.db.main.api.share_mapping_update') as mock_upd:
            ...
            share_mapping_libvirt_nfs.umount()
