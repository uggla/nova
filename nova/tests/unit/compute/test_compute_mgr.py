...
class ComputeManagerUnitTestCase(test.NoDBTestCase,
    ...
    def test_init_instance_with_shares(self):
        """Ensure that the shares linked to the instance are successfully
        mounted when the service initiates.
        """
        instance = fake_instance.fake_instance_obj(
                self.context,
                uuid=uuids.instance,
        ...
        with test.nested(
            ...
            mock.patch.object(instance, 'save', autospec=True),
            mock.patch("nova.compute.manager.ComputeManager._get_share_info"),
            mock.patch("nova.compute.manager.ComputeManager.mount_share"),
            mock.patch(
                "nova.compute.manager.ComputeManager._resume_guests_state"),
        ) as (
                mock_get_power_state,
                mock_nw_info,
                mock_instance_save,
            ...
            mock_resume_guest.assert_called_once_with(
                self.context, instance, mock.ANY)
            self.assertIsNone(instance.task_state)

    def test_init_instance_with_shares_in_error(self):
        """Ensure that the shares linked to the instance in error log
        an error.
        """
        instance = fake_instance.fake_instance_obj(
                self.context,
        ...
        with test.nested(
            ...
            mock.patch("nova.compute.manager.ComputeManager._get_share_info"),
            mock.patch("nova.compute.manager.ComputeManager.mount_share"),
            mock.patch("nova.compute.manager.LOG.error", autospec=True),
            mock.patch(
                "nova.compute.manager.ComputeManager._resume_guests_state"),
        ) as (
                mock_get_power_state,
            ...
            mock_shares.return_value = share_info
            mock_mount.side_effect = exception.ShareMountError(
                share_id=share_mapping["uuid"],
                server_id=share_mapping["instance_uuid"],
                reason="fake_mount_error",
            )
            self.compute._init_instance(self.context, instance)
            mock_log.assert_called_once_with(
                ...
                {
                    "share_id": share_mapping["uuid"],
                    "instance_uuid": share_mapping["instance_uuid"],
                },
            )
            # mock_mount.assert_called_once_with(
            #     self.context, instance, share_info[0])
    ...
    @mock.patch('nova.objects.ShareMappingList.get_by_instance_uuid')
    def test_get_share_info_fails_share_mapping_in_error(
        self, mock_db, mock_instance
    ):
        self.flags(shutdown_retry_interval=20, group='compute')
        instance = fake_instance.fake_instance_obj(
                self.context,
        ...
        self.assertIn(
            "Share 232a4b40-306b-4cce-8bf4-689d2e671553 used by "
            "instance 1 is in error state.",
            exc.message,
        )
    ...
    def test_mount_nfs_share(
        self, mock_get_access, mock_allow, mock_drv, mock_db
    ):
        self.flags(shutdown_retry_interval=20, group='compute')
        instance = fake_instance.fake_instance_obj(
                self.context,
        ...
        mock_allow.assert_called_once_with(
            share_mapping.share_id, 'ip', compute_ip, 'rw')
    ...
    def test_mount_cephfs_share(
        ...
        mock_allow.assert_called_once_with(
            share_mapping.share_id, 'cephx', 'nova', 'rw')
        mock_drv.assert_called_once_with(self.context, instance, share_mapping)
    ...
    def test_umount_nfs_share(
        ...
        compute_ip = CONF.my_block_storage_ip
        share_mapping = self.get_fake_share_mapping()
        self.compute.umount_share(self.context, instance, share_mapping)
        mock_drv.assert_called_once_with(self.context, instance, share_mapping)
    ...
    @mock.patch("nova.compute.manager.ComputeManager.stop_instance")
    @mock.patch(
        "nova.compute.manager.ComputeManager._get_share_info",
        return_value=objects.ShareMappingList(),
    )
    def test_init_instance_retries_power_off(self, mock_shares, mock_stop):
        instance = objects.Instance(self.context)
        instance.uuid = uuids.instance
        instance.id = 1
        instance.vm_state = vm_states.ACTIVE
    ...
    @mock.patch("nova.compute.manager.ComputeManager.start_instance")
    @mock.patch(
        "nova.compute.manager.ComputeManager._get_share_info",
        return_value=objects.ShareMappingList(),
    )
    def test_init_instance_retries_power_on(self, mock_shares, mock_start):
        instance = objects.Instance(self.context)
        instance.uuid = uuids.instance
        instance.id = 1
        instance.vm_state = vm_states.ACTIVE
    ...
    @mock.patch(
        "nova.compute.manager.ComputeManager.start_instance",
    ...
    def test_init_instance_retries_power_on_silent_exception(
        self, mock_shares, mock_start
    ):
        instance = objects.Instance(self.context)
        instance.uuid = uuids.instance
        instance.id = 1
    ...
    @mock.patch(
        "nova.compute.manager.ComputeManager.stop_instance",
        return_value=Exception,
    )
    @mock.patch(
        "nova.compute.manager.ComputeManager._get_share_info",
    ...
    def test_init_instance_retries_power_off_silent_exception(
        self, mock_shares, mock_stop
    ):
        instance = objects.Instance(self.context)
        instance.uuid = uuids.instance
        instance.id = 1
