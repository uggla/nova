...
class ShelveComputeManagerTestCase(test_compute.BaseTestCase):
        ...
        if clean_shutdown:
            if guest_power_state == power_state.PAUSED:
                mock_power_off_call_list.append(
                        mock.call(self.context, instance, 0, 0, mock.ANY))
            else:
                mock_power_off_call_list.append(
                    mock.call(self.context, instance, CONF.shutdown_timeout,
                              CONF.compute.shutdown_retry_interval, mock.ANY))
        else:
            mock_power_off_call_list.append(mock.call(self.context,
                instance, 0, 0, mock.ANY))
    ...
    def test_shelve_offload(self, mock_power_off):
        ...
        mock_power_off.assert_called_once_with(
            self.context, instance, CONF.shutdown_timeout,
            CONF.compute.shutdown_retry_interval, mock.ANY)
    ...
    def test_unshelve_with_resource_request(
        ...
        self.compute.unshelve_instance(
            self.context, instance, image=None,
            filter_properties={}, node='fakenode1', request_spec=request_spec,
            accel_uuids=[])
    ...
    def test_unshelve_spawn_fails_cleanup_instance_image_ref(self):
        ...
        with mock.patch.object(self.compute.driver, 'spawn',
            ...
            self.assertRaises(test.TestingException,
                    ...
                    node='fakenode1', request_spec=fake_spec, accel_uuids=[])
