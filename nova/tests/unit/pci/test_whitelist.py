...
class WhitelistTestCase(test.NoDBTestCase):
    ...
    def test_device_tags(self, case):
        wl = (
        ...
        parsed = whitelist.Whitelist([white_list])
        self.assertEqual(1, len(parsed.specs))
        self.assertEqual(parsed.specs[0].tags[case["tag"]], case["expected"])

    @ddt.data({"tag": "managed"},
              {"tag": "live_migratable"})
    def test_device_managed_not_set(self, case):
        white_list = (
            '{"product_id":"0001", "vendor_id":"8086"}'
        )
        parsed = whitelist.Whitelist([white_list])
        self.assertEqual(1, len(parsed.specs))
        self.assertNotIn(case["tag"], parsed.specs[0].tags)

    @ddt.data({"tag": "managed"},
              {"tag": "live_migratable"})
    def test_device_managed_invalid_value(self, case):
