...
class ServerSharesTest(BaseTestCase):
    ...
    @mock.patch('nova.compute.api.API.mount_share')
    @mock.patch(
        'nova.virt.hardware.check_shares_supported', return_value=None
    )
    @mock.patch(
    ...
    def test_create(
        ...
        self.assertEqual(mock_mount.call_args.args[1].uuid, instance.uuid)
        self.assertIsInstance(
            mock_mount.call_args.args[2], objects.share_mapping.ShareMapping)
        self.assertEqual(
            mock_mount.call_args.args[2].share_id, fake_db_share['share_id'])
    ...
    def test_delete(
        ...
        self.assertIsInstance(
            mock_umount.call_args.args[1], objects.instance.Instance)
        self.assertEqual(mock_umount.call_args.args[1].uuid, instance.uuid)
        self.assertIsInstance(
            mock_umount.call_args.args[2], objects.share_mapping.ShareMapping)
        self.assertEqual(
            mock_umount.call_args.args[2].share_id, fake_db_share['share_id'])
