...
class PciDeviceStats(object):
    ...
    def _filter_pools_for_live_migratable_devices(
        ...
        :returns: A list of pools that can be used to support the request if
            this is possible.
        """
        # TODO(uggla) check this code is working without pci in placement.
        #import pdb; pdb.set_trace()
        if all(spec.get("live_migratable") == 'yes' for spec in request.spec):
            # if all specs require live migratable devices, then we need to
            # reduce the pools by the ones that support them.
            pools = [pool for pool in pools if pool.get("live_migratable")
                     and pool['live_migratable'] == 'yes']
        elif all(spec.get("live_migratable") == 'no' for spec in request.spec):
    ...
    def _filter_pools(
        ...
        # TODO(Uggla): Inject the check for LM
        before_count = after_count
